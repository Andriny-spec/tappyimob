  ...
  try {
    ...
    if (!site) {
      return {
        ...
        description: 'O site requisitado não foi encontrado.'
      };
    }

    return {
      title: site.nome,
      description: site.descricao || `${site.nome} - Portal imobiliário`
    };
  } catch (error) {
    console.error('Erro ao buscar metadados:', error);
    return {
      title: 'Erro ao carregar o site',
      description: 'Ocorreu um erro ao tentar carregar as informações do site.'
    };
  }
}

// Componente servidor assíncrono
export default async function ImobiliariaHomePage({
  params,
}: {
  params: { imobiliaria: string };
}) {
  ...
  console.log('Renderizando página para imobiliária:', imobiliariaSlug);
  ...
  try {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4">
        ...
        <p className="text-gray-600 mb-4">Não foi possível carregar os dados do site</p>
      </div>
    );
  }
}
